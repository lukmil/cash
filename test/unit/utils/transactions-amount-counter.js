...
import transactionsAmountCounter from '../../../app/utils/transactions-amount-counter';
...
describe('Transactions amount counter since start of week', () => {
  it('end date is null', () => {
    const block = () => transactionsAmountCounter.sinceStartOfWeek(null, []);
    assert.throws(block, Error('end date is not given'));
  });

  it('end date is not valid', () => {
    const block = () => transactionsAmountCounter.sinceStartOfWeek('abc', []);
    assert.throws(block, Error('end date is not valid'));
  });

  it('no transactions given', () => {
    const actual = transactionsAmountCounter.sinceStartOfWeek(date20191014, null);
    assert.strictEqual(actual, 0);
  });

  it('empty transactions given', () => {
    const actual = transactionsAmountCounter.sinceStartOfWeek(date20191014, []);
    assert.strictEqual(actual, 0);
  });

  it('no transactions since start of week', () => {
    const actual = transactionsAmountCounter.sinceStartOfWeek(date20191013, transactions);
    assert.strictEqual(actual, 0);
  });

  it('end date matches start of week ', () => {
    const actual = transactionsAmountCounter.sinceStartOfWeek(date20191014, transactions);
    assert.strictEqual(actual, 100, 'included monday transaction');
  });

  it('end date is tuesday', () => {
    const actual = transactionsAmountCounter.sinceStartOfWeek(date20191015, transactions);
    assert.strictEqual(actual, 300);
  });

  it('end date is sunday', () => {
    const actual = transactionsAmountCounter.sinceStartOfWeek(date20191020, transactions);
