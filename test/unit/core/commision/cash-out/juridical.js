...
describe('Cash out juridical commissions', () => {
  it('commission is less than min amount', () => {
    const actual = juridical.count(createTransaction(10), config.cashOut.juridical);
    assert.strictEqual(actual, 0.5);
  });

  it('commission is greater than min amount', () => {
    const actual = juridical.count(createTransaction(1000), config.cashOut.juridical);
